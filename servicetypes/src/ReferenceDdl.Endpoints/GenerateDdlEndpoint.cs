...
using ReferenceDdl.Endpoints.Logging;
...
public sealed class GenerateDdlEndpoint : Endpoint<GenerateDdlRequest, GenerateDdlResponse>
{
        ...
        string? schemaFilter)
    {
        ...
        foreach (var group in groups)
        {
            ...
            sb.AppendLine(FormattableString.Invariant($"-- Schema: {group.Key}"));

            foreach (var container in group)
            {
                sb.AppendLine(FormattableString.Invariant($"CREATE TABLE {quoteOpen}{group.Key}{quoteClose}.{quoteOpen}{container.Name}{quoteClose} ("));
                ...
                for (var i = 0; i < fields.Count; i++)
                {
                    ...
                    sb.AppendLine(FormattableString.Invariant($"    {quoteOpen}{field.Name}{quoteClose} {field.FieldType.TypeName}{identity} {nullable}{separator}"));
                }
                ...
                if (pkFields.Count > 0)
                {
                    ...
                    sb.AppendLine(FormattableString.Invariant($"    ,CONSTRAINT {quoteOpen}PK_{container.Name}{quoteClose} PRIMARY KEY ({pkCols})"));
                }
