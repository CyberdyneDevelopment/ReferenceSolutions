...
{
    var startupLogger = LoggerFactory.Create(l => l.AddConsole()).CreateLogger("Reference.Management.UI.Tailwind.Startup");
...
PlatformServices.Initialize(app);
...
app.MapPost("/auth/login", async (HttpContext context, IHttpClientFactory httpClientFactory, ILogger<Program> logger) =>
{
...
app.MapGet("/auth/logout", async (HttpContext context, IHttpClientFactory httpClientFactory, ILogger<Program> logger) =>
{
